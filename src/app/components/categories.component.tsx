		...
		<div className='flex gap-3 mt-16'>
			{categories.length === 0
				? [...Array(4)].map((_, index) => (
						<div
							key={index}
							className='product cursor-pointer flex items-center flex-col p-6 border-[1px] rounded-xl w-64 h-36 animate-pulse'
						>
				...
				: categories.map((category, index) => (
						<Link
							key={index}
							href={`/catalog?c=${category.title.toLowerCase()}`}
						>
							<div
								className={`product cursor-pointer flex items-center flex-col p-6 border-[1px] dark:border-[#474747] rounded-xl w-64 h-36 ${kanitMini.className}`}
							>
								<p
									className={`${kanit.className} text-xl w-full flex justify-center text-center`}
								>
									{category.title}
								</p>
								<p
									className={`w-full mt-2 flex justify-center text-center text-gray-400`}
								>
									{category.subtitle}
								</p>
