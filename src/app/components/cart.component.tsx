...
const Cart = ({ cartMenu, setCartMenu }: CartProps) => {
	...
	return (
		<div
			...
			} fixed rounded-l-xl z-10 bg-backgroundColor border border-gray-300 dark:border-[#3d3d3d] w-1/3 h-1/2 right-0 top-24 transition-transform duration-300`}
			role='dialog'
		...
		>
			<p
				id='cart-heading'
				className='text-3xl mt-2 flex justify-center text-center items-center'
			>
				Cart
			</p>
			...
			{cart.length === 0 ? (
				...
				<>
					<div className='flex w-full h-2/3 flex-col mt-2 gap-3 overflow-y-auto'>
						{cart.map(item => (
							<div
					...
					<div className='flex text-xl justify-center items-center'>
						<div className='flex gap-5 items-center'>
							...
							<button
								...
								className='fillButton rounded-2xl px-5 py-2'
							>
								Place an order
							</button>
						</div>
